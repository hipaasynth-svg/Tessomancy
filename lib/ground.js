...
export async function runGroundedReason(question, referenceData, reframedQuestion) {
  ...
  let parsed = null;
  try {
    // Token ceiling for the grounded search step. This must clear not just the
    // final JSON but everything the model emits *before* it — interleaved
    // reasoning, several web_search rounds, and (on current models) code
    // execution. Set too low, the response is cut off (stop_reason=max_tokens)
    // mid-reasoning, before any JSON, and the truncated text parses as null —
    // which reads downstream as "no data found" and goes silently silent. This
    // is the same failure 631fe87 fixed by raising the ceiling; the Claude
    // Sonnet reasoning model needs far more headroom than the 2500 that value
    // assumed, so give it room to actually reach the JSON. (Verified: at 2500
    // the localized-prostate-cancer question truncates before the JSON; at 8000
    // it returns a clean 4-outcome object.)
