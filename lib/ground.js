...
export async function runGroundedReason(question, referenceData, reframedQuestion) {
  ...
  let parsed = null;
  try {
    // Token ceiling (8000): this must clear not just the final JSON but
    // everything the model emits *before* it — interleaved reasoning, the
    // web_search rounds, and (on current models) code execution. Set too low,
    // the response is cut off (stop_reason=max_tokens) mid-reasoning, before any
    // JSON, and the truncated text parses as null — which reads downstream as
    // "no data found" and goes silently silent. This is the same failure
    // 631fe87 fixed by raising the ceiling; the Sonnet reasoning model needs far
    // more headroom than the old 2500 (verified: the localized-prostate-cancer
    // question truncates before the JSON at 2500, returns a clean 4-outcome
    // object at 8000).
    //
    // NOTE ON LATENCY: this grounded search is the only slow step on the request
    // path, and it does NOT fit the 60s app/api/verdict budget — spoken results
    // measured 135–290s end to end. max_uses is left at the callAnthropicSearch
    // default on purpose: capping it lower (tried 3) makes calls end sooner by
    // simply refusing to ground an answer — it trades timeout risk for exactly
    // the silence this whole change exists to remove. The real lever is
    // maxDuration, not fewer searches; see app/api/verdict/route.js.
    const raw = await callAnthropicSearch(MODELS.ground, GROUND_SYSTEM, user, 8000);
