...
export async function runGroundedReason(question, referenceData, reframedQuestion) {
  ...
  let parsed = null;
  try {
    const raw = await callAnthropicSearch(MODELS.ground, GROUND_SYSTEM, user, 2500);
    ...
    if (!parsed && raw) {
      const repaired = await callAnthropic(
        MODELS.render,
        `The text below was supposed to contain exactly one strict JSON object (keys like "speak", "outcomes", "confidence", "basis", "mirror_candidates"). Extract or repair that object and return ONLY the JSON, nothing else. Do not invent data that is not in the text. If no object is recoverable, return {"speak": false}.`,
        raw,
        1500
      );
