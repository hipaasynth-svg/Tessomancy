...
export async function getAccessStatus({ deviceToken, customerId }) {
  ...
  const [freeUsed, balance, sub, usage] = await Promise.all([freeUsedP, balanceP, subP, usageP]);
  ...
  return {
    ...
    subscription,
    hasAccess: Boolean(!freeUsed || Number(balance) > 0 || (subscription && !subscription.hardCapped)),
    tiers: publicTiers(),
  };
}
...
// Priority: active subscription (under hard cap) > pack balance > weekly free taste.
