...
export async function getAccessStatus({ deviceToken, customerId }) {
  ...
  return {
    ...
    hasAccess: Boolean(devBypass || unlimited || !freeUsed || Number(balance) > 0 || (subscription && !subscription.hardCapped)),
    tiers: publicTiers(),
  };
}
...
export async function checkAndConsumeAccess({ deviceToken, customerId }) {
  if (isDevBypass()) {
    return { allowed: true, source: "dev_bypass" };
  }
...
export async function refundAccess(access, { deviceToken, customerId }) {
  try {
    ...
    if (access.source === "free" && deviceToken) {
      ...
      await r.incrby(k.balance(customerId), 1);
    } else if (access.source === "subscription" && customerId) {
      const usage = await r.get(k.usage(customerId));
      if (usage && usage.count > 0) {
        await r.set(k.usage(customerId), { ...usage, count: usage.count - 1 });
      }
    }
